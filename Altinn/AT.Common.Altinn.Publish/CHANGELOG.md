...
- Granular registration. `services.AddAltinn(...)` returns a builder on which each API is opted into individually — `AddStorage()`, `AddEvents()`, `AddApps()`, `AddCorrespondence()`, `AddDialogporten()`, `AddAltinnAdapter()`, `AddMeldingerAdapter()` — so an application only registers, and only authenticates against, the APIs it uses. `AddAllClients()` registers them all. Adding a client twice is a no-op, and configuration is independent of call order, so a client pulled in by an adapter can still be configured afterwards.
