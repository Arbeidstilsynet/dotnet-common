...
internal static class InstanceQueryParametersExtensions
{
    ...
    private static string? GetQueryParameterName(this PropertyInfo propertyInfo)
    {
        if (
            propertyInfo.GetCustomAttribute<MappedQueryParameterAttribute>()
            is not { QueryParameterName: { Length: > 0 } queryParameterName }
        )
        {
            return null;
        }
    ...
    private static string? GetHeaderParameterName(this PropertyInfo propertyInfo)
    {
        if (
            propertyInfo.GetCustomAttribute<MappedRequestHeaderParameterAttribute>()
            is not { HeaderParameterName: { Length: > 0 } headerParameterName }
        )
        {
            return null;
        }
