...
- **BREAKING**: `InstanceQueryParameters` is replaced by `InstanceQuery`, which composes the generated query parameters with the `X-Ai-InstanceOwnerIdentifier` header rather than mirroring them. Parameters added to the storage specification now appear as soon as the client is regenerated. `AltinnDateTimeQuery` becomes a set of helpers producing the comparison expressions the generated parameters take as strings.
...
- The generated models are Kiota `IParsable` types, not `System.Text.Json` POCOs. Deserialising them with `JsonSerializer` silently yields empty objects, and serialising them with it does not reproduce Altinn's JSON: enums become integers, unset properties are emitted as nulls, and free-form maps such as `Instance.DataValues` gain an extra `additionalData` level that no serializer option removes. The README covers this under "Returning generated models from your own API".
