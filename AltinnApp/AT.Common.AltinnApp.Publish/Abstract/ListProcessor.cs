    ...
    where TDataModel : class
{
    ...
    {
        if (typeof(TListItem).GetProperty("AltinnRowId") is not {} propertyInfo || propertyInfo.PropertyType != typeof(Guid))
        {
            throw new InvalidOperationException(
                $"Type {typeof(TListItem).Name} must have a property named 'AltinnRowId' of type Guid. This should be auto-generated by Altinn for types used in repeating groups in the data model."
            );
        }
    }
    ...
    {
        var prevItems = previousList?.ToDictionary(item => item.GetAltinnRowId(), item => item) ?? [];
        var currItems = currentList?.ToDictionary(item => item.GetAltinnRowId(), item => item) ?? [];
...
file static class Extensions
{
    ...
    {
        var propertyInfo = typeof(TListItem).GetProperty("AltinnRowId");
        if (propertyInfo is null || propertyInfo.PropertyType != typeof(Guid))
        {
            throw new InvalidOperationException(
                $"Type {typeof(TListItem).Name} must have a property named 'AltinnRowId' of type Guid."
            );
        }
        ...
        {
            throw new InvalidOperationException(
                $"The 'AltinnRowId' property of item of type {typeof(TListItem).Name} cannot be null."
            );
        }
